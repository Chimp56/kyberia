...
Generate separate Ed25519 key pairs with an approved secret-management tool. Store DER PKCS#8 private keys and DER SPKI public keys as base64 environment secrets. The coordinator has its own private key and each runner has only that coordinator's public key. Each runner's private key stays on that host; the coordinator has only the pinned host public key. Rotate by adding a new key ID/config, draining old jobs, then removing the old secret. Never commit key material.

Copy and edit `config.example.json` and `runner.example.json`. Every coordinator operation points to the fixed runner entry point. The runner independently maps the signed suite and bounded parameters to its administrator-owned static command table. It verifies the coordinator signature, key ID, freshness, nonce uniqueness, exact checked-out revision and timeout before execution. Unix descendants are held in a process group. The retained fixed-catalog Windows helper uses Kyberia's tested Job Object primitive, but automatic command review did not permit connecting a helper-mediated process path during this run. The TypeScript runner therefore fails closed on Windows; connecting the fixed helper and native execution remain an explicit Windows review/runtime gate.
...
Only UTF-8 text summaries and logs use artifact class `kyberia-lab-text-v1`. The sanitizer removes MAC addresses, IPv4 addresses, SSID values and common secret fields and applies byte caps. Raw PCAP, PCAPNG, KismetDB, spectrum IQ, location traces and arbitrary files are never returned by this server.
