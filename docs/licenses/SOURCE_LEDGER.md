...
CI uses GitHub's [actions/checkout](https://github.com/actions/checkout) at immutable commit `d23441a48e516b6c34aea4fa41551a30e30af803` (v6 tag resolved through the GitHub API) and [actions/setup-python](https://github.com/actions/setup-python) at immutable commit `e797f83bcb11b83ae66e0230d6156d7c80228e7c` (v6.0.0); the general workflow pins CPython 3.12.10 on every matrix host. The official `actions/python-versions` manifest advertises that patch release for Linux, macOS and Windows runners, while 3.12.12 is absent for the current macOS and Windows runner images. The separately audited Sionna proof remains pinned to Python 3.12.12. Their LICENSE files at those commits declare MIT. The actions execute unmodified on hosted CI runners and are not bundled in Kyberia; their own bundled dependency notices remain upstream. Update only after reviewing the new commits, licenses and runtime compatibility. Credentials are not persisted, job permissions are read-only, and repository cleanup is disabled. Hosted workflow execution has not yet supplied the second-platform validation gate.
