...
| Zod | 4.1.11 | MIT | Development-only Lab MCP dependency | Strict untrusted MCP/config/runner schemas, unmodified npm package | Update with MCP SDK compatibility review and strict-schema regressions |
| TypeScript / tsx / Prettier / Node type declarations | 5.9.3 / 4.20.6 / 3.6.2 / 24.10.0 | Apache-2.0 / MIT / MIT / MIT | Build and test tooling only | Exact development pins in the Lab MCP lockfile; not part of Kyberia product runtime | Update together with Node pin; format, compile and run the complete Lab MCP contract suite |
