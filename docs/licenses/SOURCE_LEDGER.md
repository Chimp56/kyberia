...
| Zod | 4.1.11; bundled notice SHA-256 `3f1189b28e3866e0d979968d466b78f813f76827cfdca1fbb124cc0a5c8841f8` | MIT | Lab MCP dependency bundled into the content-addressed runner; `ZOD-LICENSE` ships with the package | Strict untrusted MCP/config/runner schemas, unmodified npm package bundled by esbuild | Update with MCP SDK compatibility review, notice hash review, bundle rebuild and strict-schema regressions |
| TypeScript / tsx / esbuild / Prettier / Node type declarations | 5.9.3 / 4.20.6 / 0.25.12 / 3.6.2 / 24.10.0 | Apache-2.0 / MIT / MIT / MIT / MIT | Build and test tooling; esbuild produces the self-contained Lab runner bundle | Exact development pins in the Lab MCP lockfile; generated bundle contains only Kyberia runner code and bundled Zod validation | Update together with Node pin; format, compile, rebuild the bundle, verify deterministic bytes and run the complete Lab MCP contract suite |
| Lab MCP npm transitive graph | `tools/lab-mcp/pnpm-lock.yaml` lockfile v9; per-package versions and registry SHA-512 integrity | Per-package upstream licenses; exact installed notices remain authoritative | Development-only coordinator/runner package; code payload contains only `dist/src`, accompanied by safe configuration schematics, README and schema notices | Deterministic CycloneDX inventory in `tools/lab-mcp/dependency-sbom.cdx.json`, generated from the structurally parsed lock and installed license metadata | Run `npm run sbom`, `npm run check`, then a network-authorized `pnpm audit --prod`; review license notices before any redistribution |
| YAML / Ajv / Ajv Formats | 2.8.1 / 8.20.0 / 3.0.1 | ISC / MIT / MIT | Development-only SBOM validation | Structurally parse the pinned pnpm lock and validate the generated document; exact npm integrity values are in `tools/lab-mcp/pnpm-lock.yaml` | Update exact pins, regenerate SBOM and rerun direct/transitive-coordinate plus official-schema tests |
| CycloneDX JSON schemas | CycloneDX 1.6 BOM `3e92dddbc30cf7f6a02b80f0942b1a4cfd4fb1c26f1dfc4310afa9d613cafb93`; JSF 0.82 `8bae002c25e723db7ee1f26afde680ae1a2b1a8f6b4b4b0fd65dc3becb090aae`; SPDX helper `ea6e844ee6fba1e93473d94834d0ee0996970533497935f932f73d488ffdf4a3`; notice `6c29f22a4a7385285c6f579ec9f33c5e989f00739d6b257243a0b082ec9447ae` | Apache-2.0; vendored schema notice retained in source and package | Development/package validation; redistribution permitted with notices | BOM schema downloaded from `https://github.com/CycloneDX/specification/blob/1.6/schema/bom-1.6.schema.json`; referenced JSF/SPDX schemas retained from the repository's audited CycloneDX schema set | Fetch the same upstream tagged schema set, verify hashes/license, replace all three together, validate a regenerated SBOM |
| Kismet | audit 2d25ad004e9216ac963c4f156e9077331717959c; DB contract schemas 5–10; REST status contract reviewed 2026-09-08 | GPL-2.0-or-later per supplied audit; verify distribution components | External only; no source/runtime bundled | Independent SQLite reader and typed REST status projection based on primary field documentation; original synthetic fixtures, no live Kismet execution yet | Pin tested releases, authenticate/probe, API/DB/PCAP parity and legal review before optional bundling |
...
Kismet live-status fixtures in `crates/kismet-adapter/tests/live_status.rs`
are original synthetic HTTP responses and local TCP exchanges based on the
official REST field definitions and the pinned implementation packet. They
contain no Kismet runtime, capture, network identifier or copied upstream
fixture. Redistribution remains `NOASSERTION`; update the fixture contract,
secret-leak checks and exact-path assertions together after reviewing a pinned
Kismet server response. Their file hash is intentionally not added to the
general numerical fixture ledger because that ledger covers only
`tools/validation/fixtures.py` assets.
...
[cargo-sources.json](cargo-sources.json) records all 223 resolved external packages, including development and conditional platform dependencies. Each entry contains source, version, declared upstream license, archive SHA-256, redistribution qualification, transformation, provenance and update procedure. `python3 tools/source_inventory.py check` compares the inventory against locked local Cargo metadata and downloaded archives. This is a source inventory, not an assertion that all packages ship in the executable. Target-specific distributable SBOM generation and notice assembly remain release gates.

Direct dependencies are Serde 1.0.228 and serde_json 1.0.149 (MIT OR Apache-2.0), rusqlite 0.40.2 (MIT), sha2 0.10.9 (MIT OR Apache-2.0), tempfile 3.23.0 (MIT OR Apache-2.0), uuid 1.21.0 (Apache-2.0 OR MIT), and test-only proptest 1.10.0 (MIT OR Apache-2.0). The Kismet live adapter adds reqwest 0.12.28 (MIT OR Apache-2.0), with only its blocking and rustls/webpki-roots features enabled; redirects and environment proxies are disabled at runtime, and explicit address overrides prevent implicit DNS inside the bounded poll. The resolved hyper, rustls, tokio and webpki-roots packages are recorded in the generated inventory. The project-store's typed analytical adapter additionally uses Apache Arrow Rust `parquet` 59.3.0 (Apache-2.0) with its default features disabled and only the `arrow` feature enabled, plus direct `arrow-array` 59.3.0 (Apache-2.0 AND MIT), `arrow-schema` 59.3.0 (Apache-2.0) and `bytes` 1.12.1 (MIT). The registry sources are unmodified; the lockfile and generated cargo inventory include all resolved packages, including conditional platform dependencies. Kyberia crate license identifiers are the provisional permissive distribution choice; no distribution clearance or legal grant is inferred from package metadata alone.
...
CI uses GitHub's [actions/checkout](https://github.com/actions/checkout) at immutable commit `d23441a48e516b6c34aea4fa41551a30e30af803` (v6 tag resolved through the GitHub API), [actions/setup-python](https://github.com/actions/setup-python) at immutable commit `e797f83bcb11b83ae66e0230d6156d7c80228e7c` (v6.0.0), and [actions/setup-node](https://github.com/actions/setup-node) at immutable commit `49933ea5288caeca8642d1e84afbd3f7d6820020` (v4 tag resolved through the GitHub API); the general workflow pins CPython 3.12.10 and Node 24.20.0 on every matrix host. The official `actions/python-versions` manifest advertises that Python patch release for Linux, macOS and Windows runners, while 3.12.12 is absent for the current macOS and Windows runner images. The separately audited Sionna proof remains pinned to Python 3.12.12. Their LICENSE files at those commits declare MIT. The actions execute unmodified on hosted CI runners and are not bundled in Kyberia; their own bundled dependency notices remain upstream. Update only after reviewing the new commits, licenses and runtime compatibility. Credentials are not persisted, job permissions are read-only, and repository cleanup is disabled. Hosted workflow execution has not yet supplied the second-platform validation gate.
...
The renderer's original Rust scene generator locks 21 external packages already present with identical versions, registry sources and checksums in the workspace Rust inventory. The WASM admission crate uses the primary workspace lockfile after the cross-checkout reproducibility correction in ADR-0026. These support crates add no external package versions. The checked-in validator is built from Kyberia source plus those locked dependencies; its exact digest is retained in `research/renderer/wasm/kyberia_scene_validator_wasm.wasm.sha256`. Generated scene fixtures are original synthetic Kyberia data, with no vendor or competitor source. Rebuild with `npm run check:wasm` from `research/renderer`, regenerate fixtures using the locked support crate when contracts change, and repeat canonical admission, numerical, browser and license verification. Distribution still requires complete notices and a distributable SBOM; this research artifact is not release approval.
