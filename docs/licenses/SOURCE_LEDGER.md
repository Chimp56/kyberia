...
| Lab MCP npm transitive graph | `tools/lab-mcp/pnpm-lock.yaml` lockfile v9; per-package versions and registry SHA-512 integrity | Per-package upstream licenses; exact installed notices remain authoritative | Development-only coordinator/runner package; runtime payload contains only `dist/src` plus npm-required package metadata/readme | Deterministic CycloneDX inventory in `tools/lab-mcp/dependency-sbom.cdx.json`, generated from the full lock graph | Run `npm run sbom`, `npm run check`, then a network-authorized `pnpm audit --prod`; review license notices before any redistribution |
