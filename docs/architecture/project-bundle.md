...
The `kyberia-project-store` adapter owns filesystem, SQLite and immutable Parquet chunk side effects. It imports canonical project identity from `kyberia-domain`; the domain does not import storage or analytical-library types. This is the initial container plus FND-006 chunk proof for plan §§11, 18.1 and Gate C; Gate C's portability, workload and independent review gates remain open.

A project is a newly reserved directory containing `project.sqlite`, `manifest.json`, and `artifacts/<lowercase-sha256>`. SQLite stores the committed versioned manifest, chunk descriptors and observation-ID uniqueness index in one transaction. JSON is a redundant, human-readable projection; canonical observation chunks are real typed Parquet files using the fixed nullable V2 envelope superset and its schema fingerprint. Artifact bytes are immutable, bounded to 64 MiB per chunk, and checksummed before use. The manifest is bounded to 4 MiB and 10,000 entries. Formats requiring larger data must chunk through their owning adapter; this interface never silently truncates input.

Artifact imports persist and sync bytes before registration. Generic imports reject the normalized-observation kind; `publish_observation_chunk` is the only path that creates the descriptor and member index required for a valid chunk. A write transaction rechecks the current physical schema, singleton record and revision, requires exactly one changed row, and reads back the authoritative manifest and exact serialized bytes before publishing the projection while holding the writer lock and committing. Failed registration can leave an unreferenced blob. A crash between projection publication and commit can leave a stale projection; `verify` diagnoses it and explicit `recover-manifest` rebuilds it from committed SQLite. Recovery does not repair corrupted evidence and exits unsuccessfully if verification still fails. Creation reserves a fresh directory and retains partial files on failure for diagnosis.
