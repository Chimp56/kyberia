...
Plan version 0.3 has 6,210 lines, 447 headings, and 438 explicit requirement/ADR definition occurrences. They become 5,396 blocks: headings, prose paragraphs, individual list items, individual table rows, fenced examples/diagrams, separators, and whitespace. Concatenating their exact `source.text` fields reconstructs `plan.md`. The checker independently re-extracts the plan and compares every block, range, hash, heading ancestry, and inventory. Omissions, additions, edited text, changed ancestry, and stale hashes fail.
