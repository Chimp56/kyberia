...
| Kyberia Lab MCP | Security-correction worktree `feat/lab-mcp` | Production stdio coordinator/runner path, credential allowlist, complete immutable commit snapshots, all code-bearing argument and executable SHA-256 pins, distinct key-role identities, offline-verifiable safe host evidence, seed binding, durable recovery, hard cancellation deadline, bounded redaction, official CycloneDX 1.6 validation and package/CI gates pass locally; independent rereview and physical Windows/Kismet/CUDA/spectrum gates remain |
