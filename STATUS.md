...
| Phase 7 Sionna CPU convergence support diagnostic | Author worktree `feat/phase7-cpu-convergence-diagnostics` | Phase 7 remains IN_PROGRESS. Bounded repeated-seed/sample-budget summarizer reuses ordinary `radio_map` requests, validates worker provenance, propagates cancellation, and preserves worker cleanup diagnostics. Synthetic algorithm tests do not execute Sionna or establish convergence; pinned runtime, held-out P2/P1 evidence, calibrated uncertainty, cross-host variance, explainable path diagnostics, Wi-Fi recomposition and Phase 7 exit remain open; see [validation](docs/validation/sionna-convergence-diagnostic.md) |
