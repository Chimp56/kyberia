...
| WIFI-001 bounded Beacon/Probe IE parser | Integration candidate `7133e25` on isolated `integrate/wifi-ie-main-v1` | Adapted onto `main` at `0595313`; parser, tcpdump, deterministic mutation, formatting, strict Clippy, ledger, architecture and source-inventory checks pass. Fresh independent review is required before integration to `main`. Coverage-guided campaign is prior hash-matched evidence, not rerun here; other WIFI items, `INS-005`, Phase 2 exit, Kismet/physical capture and broader TST-002 gates remain open |
