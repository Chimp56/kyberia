...
- Current phases: **Parallel bounded implementation in Phase 0 (desktop map workflow) and Phase 1 (survey-session persistence and manual paths)**.
- Current Phase 0 iteration: **6 — Lab validation, desktop shell integration, and hosted Windows closure**.
...
The zero-allocation `ManualPathSurvey::observation_ids()` iterator and focused
ordering regression are integrated at `348aec6` and independently approved in
the report recorded at `6e51355`. Focused survey tests and structural checks
pass. This increment does not introduce persistence or alter the
capture-session association contract; SUR-002 and Phase 1 remain open.
...
The bounded Phase 7 convergence support diagnostic is integrated through
`a83ec42`, with cancellation propagation at `f74595d` and preservation of
worker cleanup/cancellation failures at `0a0ee26`. Independent review reports
at `fe060de`, `4b5da95`, and `ed3b53c` record the initial cancellation finding,
...
The bounded Phase 1 application/session point-survey snapshot increment is
integrated on `main` at `2a6b4a7` and independently approved in the report
...
The Phase 2 IE-explorer core is integrated on `main` at `41e88b7`, with the
reviewed identical-singleton warning correction at `26efb81`. Independent
...
work ceiling. The integrated tests did not directly test the documented
half-millidBm tie direction. An isolated, unintegrated follow-up on
`feat/phase5-bandpower-tie-20260923` from base `a8cf7dccbe1a1c40d6adc51868e90dfe2cdcd452`
now tests exact `+0.5`/`-0.5` final-quantization inputs and adjacent
representable values through a private helper seam; it does not synthesize an
exact real-sweep logarithmic tie. Its private source refactor preserves the
public API and intended calculation semantics. Independent review and
integration remain pending; see [validation](docs/validation/phase5-spectrum-contract.md).
This calculation does not provide current/average/minimum/max-hold,
...
| Phase 5 spectrum evidence contract | Integrated on `main` at `9641431`; correction re-review `16a9111`; whole-bin power at `eb27cbf`, independently approved in `8406e06`; isolated tie-regression candidate on `feat/phase5-bandpower-tie-20260923` | The integrated contract has 19 band-power tests plus earlier sweep/signature coverage. The unintegrated follow-up directly tests exact positive/negative half-millidBm quantizer ties and adjacent representable inputs via a private seam; independent review is pending. No spectrum UI, SoapySDR/vendor adapter, equivalent-bandwidth normalization, calibration/hardware/field execution; SPE-002 and Phase 5 remain open. See [validation](docs/validation/phase5-spectrum-contract.md) |
...
| Phase 7 Sionna CPU convergence support diagnostic | Integrated through `a83ec42`; cancellation and failure-preservation fixes `f74595d` and `0a0ee26`; independent reports `fe060de`, `4b5da95`, `ed3b53c` | Fourteen synthetic tests pass. The helper reuses ordinary `radio_map` requests, validates worker provenance, propagates cancellation, and preserves worker cleanup diagnostics. No Sionna engine execution or convergence claim; pinned runtime, held-out P2/P1 evidence, calibrated uncertainty, cross-host variance, explainable path diagnostics, Wi-Fi recomposition and Phase 7 exit remain open; see [validation](docs/validation/sionna-convergence-diagnostic.md) |
...
| Phase 2 bounded PCAPNG/IEEE replay bridge | Isolated source and cancellation-test candidate committed through `d4d03da296f0ec4326f5a5801c56799e689139dc` on `feat/phase2-ie-replay-20260923`; not integrated | Fourteen synthetic tests, including cancellation after a valid staged frame; strict package Clippy, fmt, architecture, package inventory, ledger and diff checks pass. Supports explicit FCS policy, raw link type 105, bounded all-or-error staging, provenance, and accounted skip categories. Fresh independent rereview and integration remain pending; live capture/Kismet, radiotap, standards references, UI/IPC, physical validation, `INS-005` and Phase 2 exit remain open; see [validation](docs/validation/phase2-pcapng-ie-replay.md) |
