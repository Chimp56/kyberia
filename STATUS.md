...
| Kyberia Lab MCP | Security-correction worktree `feat/lab-mcp` | Production stdio coordinator/runner path, credential allowlist, complete immutable commit snapshots, executable SHA-256 pinning, signed evidence provenance, seed binding, durable recovery, hard cancellation deadline, bounded redaction and package/CI gates pass locally; independent rereview and physical Windows/Kismet/CUDA/spectrum gates remain |
