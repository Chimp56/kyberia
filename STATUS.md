...
work ceiling. A bounded follow-up tests exact `+0.5`/`-0.5`
final-quantization inputs and adjacent representable values through a private
helper seam; it does not synthesize an exact real-sweep logarithmic tie. Its
private source refactor preserves the public API and intended calculation
semantics. It is integrated at `b63aab6` and independently approved with no
findings in report `71f9de2`; see
[the review](docs/reviews/phase5-bandpower-tie-current-review.md) and
[validation](docs/validation/phase5-spectrum-contract.md).
...
| Phase 5 spectrum evidence contract | Integrated on `main` at `9641431`; correction re-review `16a9111`; whole-bin power at `eb27cbf`, independently approved in `8406e06`; tie-rounding regression at `b63aab6`, independently approved in `71f9de2` | The bounded core has 20 band-power tests plus earlier sweep/signature coverage. The tie regression tests exact positive/negative final-quantizer values and adjacent representable inputs via a private seam; it does not synthesize an exact full-sweep logarithmic tie. No spectrum UI, SoapySDR/vendor adapter, equivalent-bandwidth normalization, calibration/hardware/field execution; SPE-002 and Phase 5 remain open. See [validation](docs/validation/phase5-spectrum-contract.md) |
